...
{
    public class GetTenantByDomainIdRepository(IConfiguration configuration) : GenericRepository(configuration), IGetTenantByDomainIdRepository
    {
        #region GetTenantDBConnection 
        public async Task<GetTenantByTenantNameResponse> GetTenantDBConnection(int domainId)
        {
            ...
            try
            {
                ...
                await dbConnection.QueryAsync(ServiceConstant.SchemaMasters + ProcedureConstant.GetTenantByDomainId, parameters, sqlTransaction, commandType: CommandType.StoredProcedure);
